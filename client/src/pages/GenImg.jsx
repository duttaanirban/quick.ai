...
const GenImg = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image of a ${selectedStyle} ${input}`;
      ...
      toast.error(error?.response?.data?.error || error.message || 'Image generation failed');
    }
    setLoading(false);
  }
          ...
          </div>
          {
            ...
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <div className='mt-3 h-full'>
                <img src={content} alt="Generated_img" className='w-full h-full' />
              </div>
            )
          }
